...
const summaryCards = [
  ...
  { label: 'Đồ dùng đang cho mượn', value: '86', change: '+8.2%', icon: Box, tone: 'bg-emerald-50 text-emerald-600' },
  { label: 'Yêu cầu chờ duyệt', value: '14', change: 'Cần xử lý', icon: Clock3, tone: 'bg-amber-50 text-amber-600' },
  { label: 'Karma đã luân chuyển', value: '24,680', change: '+18.4%', icon: CircleDollarSign, tone: 'bg-violet-50 text-violet-600' },
];

const requests = [
  { borrower: 'Nguyễn Minh Anh', item: 'Máy tính Casio FX-580VN X', owner: 'Trần Quốc Bảo', date: 'Hôm nay, 09:40', status: 'Chờ duyệt' },
  { borrower: 'Lê Hoàng Nam', item: 'Bộ dụng cụ sửa xe đạp', owner: 'Phạm Gia Hân', date: 'Hôm nay, 08:15', status: 'Đã xác nhận' },
  { borrower: 'Vũ Khánh Linh', item: 'Sách Giáo trình Marketing', owner: 'Đỗ Anh Tuấn', date: 'Hôm qua, 17:20', status: 'Đã xác nhận' },
  { borrower: 'Ngô Đức Thành', item: 'Tripod điện thoại', owner: 'Mai Thanh Thảo', date: 'Hôm qua, 14:05', status: 'Đã hủy' },
];
      ...
      <main className="min-h-screen lg:pl-72">
        ...
        <div className="mx-auto max-w-7xl p-5 sm:p-8">
          ...
          <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
            {summaryCards.map(({ label, value, change, icon: Icon, tone }) => <article key={label} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><div className="flex items-start justify-between"><span className={`grid h-11 w-11 place-items-center rounded-xl ${tone}`}><Icon size={21} /></span><span className="flex items-center gap-1 text-xs font-semibold text-emerald-600"><ArrowUpRight size={14} />{change}</span></div><p className="mt-5 text-2xl font-bold tracking-tight">{value}</p><p className="mt-1 text-sm text-slate-500">{label}</p></article>)}
          </section>
          ...
          <section className="mt-6 grid gap-6 xl:grid-cols-5">
            <article className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm xl:col-span-3">
              <div className="flex items-center justify-between border-b border-slate-100 px-5 py-4"><div><h3 className="font-bold">Yêu cầu mượn gần đây</h3><p className="mt-0.5 text-xs text-slate-500">Cập nhật theo thời gian thực</p></div><button className="text-sm font-semibold text-emerald-600 hover:text-emerald-700">Xem tất cả</button></div>
              <div className="overflow-x-auto"><table className="w-full min-w-[680px] text-left text-sm"><thead className="bg-slate-50 text-xs uppercase tracking-wide text-slate-500"><tr><th className="px-5 py-3 font-semibold">Người mượn</th><th className="px-5 py-3 font-semibold">Đồ dùng</th><th className="px-5 py-3 font-semibold">Thời gian</th><th className="px-5 py-3 font-semibold">Trạng thái</th></tr></thead><tbody className="divide-y divide-slate-100">{requests.map((request) => <tr key={`${request.borrower}-${request.item}`} className="hover:bg-slate-50"><td className="px-5 py-4"><p className="font-semibold">{request.borrower}</p><p className="text-xs text-slate-500">Chủ đồ: {request.owner}</p></td><td className="px-5 py-4 text-slate-600">{request.item}</td><td className="px-5 py-4 text-slate-500">{request.date}</td><td className="px-5 py-4"><span className={`inline-flex rounded-full px-2.5 py-1 text-xs font-semibold ring-1 ring-inset ${statusStyles[request.status]}`}>{request.status}</span></td></tr>)}</tbody></table></div>
            </article>
