        ...
        <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-slate-700 dark:bg-slate-800">
          <div className="flex flex-col gap-3 border-b border-slate-100 p-5 sm:flex-row sm:items-center sm:justify-between dark:border-slate-700">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-500">
                Các giao dịch nạp Karma qua PayOS.
              </p>
            </div>
            <button
              onClick={loadPayments}
              aria-label="Làm mới lịch sử"
              className="inline-flex w-fit items-center gap-2 rounded-lg border border-slate-200 px-3 py-2 text-sm font-semibold text-slate-600 hover:border-emerald-400 hover:text-emerald-700 dark:border-slate-600 dark:text-slate-300"
            >
          ...
          {loading ? (
            ...
            <div className="p-12 text-center">
              ...
              <p className="mt-1 text-sm text-slate-500">
                Nạp lần đầu để bắt đầu hành trình trao đổi xanh.
              </p>
            </div>
          ) : (
            <div className="overflow-x-auto">
              <table className="w-full min-w-[680px] text-left text-sm">
                <thead className="bg-slate-50 text-xs uppercase tracking-wide text-slate-500 dark:bg-slate-900/40">
                  <tr>
                  ...
                  {payments.map((payment) => (
                    ...
                    >
                      ...
                      <td className="px-5 py-4 text-slate-600 dark:text-slate-300">
                        {date.format(new Date(payment.created_at))}
                      </td>
                      <td className="px-5 py-4 font-semibold text-slate-700 dark:text-slate-200">
                        {money.format(payment.amount_vnd)} đ
                      </td>
                      <td className="px-5 py-4">
                        <span className="inline-flex items-center gap-1 font-bold text-emerald-700 dark:text-emerald-400">
                          <ArrowDownLeft size={15} /> +
                          {money.format(payment.karma_received)}
                        </span>
                      </td>
                      <td className="px-5 py-4">
                        <span
                          className={`rounded-full px-2.5 py-1 text-xs font-bold ${statusClass[payment.status] || statusClass.PENDING}`}
                        >
