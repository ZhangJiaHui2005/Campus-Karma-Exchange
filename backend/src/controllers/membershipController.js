...
const getMembershipDates = (activeMembership, months) => {
  const now = new Date();
  const startAt = activeMembership && activeMembership.end_at > now
...
const parseRequest = (body) => {
  ...
  if (paymentMethod === 'KARMA'
    && (!Number.isInteger(karmaCost) || karmaCost <= 0)) return null;
  if (paymentMethod === 'PAYOS'
    && (!Number.isInteger(priceVnd) || priceVnd <= 0)) return null;
...
export const createMembership = async (req, res) => {
  ...
  try {
    const result = await prisma.$transaction(async (transaction) => {
      ...
      const { startAt, endAt } = getMembershipDates(activeMembership, request.months);
      ...
      const payment = await transaction.payments.create({
        data: {
          ...
          transaction_ref: String(`${Date.now()}${crypto.randomInt(10, 100)}`.slice(-15)),
          purpose: 'MEMBERSHIP',
          status: 'PENDING',
        },
      });
    ...
    if (result.insufficientKarma) {
      return res.status(400).json({ success: false, message: 'Không đủ Karma để đăng ký gói.' });
    }
    ...
    return res.status(500).json({ success: false, message: 'Không thể tạo Karma Pass.' });
  }
};
...
export const getCurrentMembership = async (req, res) => {
  try {
    ...
    return res.status(500).json({ success: false, message: 'Không thể tải trạng thái Karma Pass.' });
  }
};