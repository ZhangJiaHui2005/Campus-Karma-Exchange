...
const createOrderCode = () => Number(`${Date.now()}${crypto.randomInt(10, 100)}`.slice(-15));
...
export const createPayOSPaymentLink = async (payment) => {
  ...
  const result = await getPayOS().paymentRequests.create({
    ...
    description: payment.purpose === 'MEMBERSHIP'
...
export const createKarmaTopup = async (req, res) => {
  try {
    ...
    if (!Number.isInteger(amountVnd) || amountVnd <= 0
      || !Number.isInteger(karmaReceived) || karmaReceived <= 0) {
    ...
    const paymentUrl = await createPayOSPaymentLink({ ...payment, ip_address: req.ip });
    ...
    return res.status(500).json({ success: false, message: 'Không thể tạo thanh toán PayOS.' });
  }
};
...
export const processPayOSWebhook = async (webhook) => {
  ...
  return prisma.$transaction(async (transaction) => {
    ...
    if (payment.status === 'SUCCESS') return { code: '00', message: 'Already confirmed' };
    ...
    if (updatedPayments.count === 0) return { code: '00', message: 'Already confirmed' };
...
export const confirmKarmaTopup = async (req, res) => {
  try {
    ...
    if (payment.user_id !== req.user.user_id) {
      return res.status(403).json({ success: false, message: 'Không có quyền xác nhận giao dịch này.' });
    }
    if (payment.status === 'SUCCESS') {
      return res.json({ success: true, message: 'Giao dịch đã được xác nhận trước đó.', alreadyConfirmed: true });
    }

    let payosStatus;
    try {
      const payosResult = await getPayOS().paymentRequests.get(Number(orderCode));
      payosStatus = payosResult.status;
    } catch (err) {
      console.error('PayOS Get Payment Error:', err);
      return res.status(502).json({ success: false, message: 'Không thể xác minh trạng thái từ PayOS.' });
    }
    ...
    if (payosStatus === 'PAID') {
      ...
      return res.json({ success: true, message: 'Đã cộng Karma vào ví.', ...result });
    }
    ...
    if (payosStatus === 'CANCELLED') {
      ...
      return res.status(400).json({ success: false, message: 'Giao dịch đã bị hủy.', cancelled: true });
    }
    ...
    return res.status(400).json({ success: false, message: 'Thanh toán không thành công.', failed: true });
  } catch (error) {
    console.error('Confirm Karma Topup Error:', error);
    return res.status(500).json({ success: false, message: 'Không thể xác nhận giao dịch.' });
  }
};
